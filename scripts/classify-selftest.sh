...
expect no-tests-matched 2 "matched no tests" 0 "	 Executed 0 tests, with 0 failures (0 unexpected) in 0.0 seconds"
expect broke-not-failed 2 "the run broke, it did not fail" 1 "	 Executed 3 tests, with 0 failures (0 unexpected) in 0.1 seconds"
...
# 7. THE HARNESS MUST UN-EDIT THE TREE. Not a classifier case — a mutate.sh one,
#    kept here because this file is what runs in CI. The first rewire used
#    `exec` to call the classifier, which replaces the shell and SKIPS the EXIT
#    trap, leaving the mutation applied in the working tree. Every later run
#    would have measured mutated source, silently.
if command -v git >/dev/null && git rev-parse --git-dir >/dev/null 2>&1; then
    if grep -q '^exec scripts/classify-run.sh' scripts/mutate.sh; then
        echo "FAIL restore-trap: mutate.sh execs the classifier; the EXIT trap that restores the file will not run"
        FAILURES=$((FAILURES+1))
    else
        echo "ok   restore-trap"
    fi
fi
